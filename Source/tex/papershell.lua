...
function M.generate(kv_raw)
  ...
  local pre_tpl_f, er1 = template.compile_file("tpl/"..pub.."/preamble.tpl", kv)
  if pre_tpl_f then
    spit("gen/preamble.inc.tex", pre_tpl_f)
  else
    spit("gen/preamble.inc.tex", "\\message{" .. er1:gsub("{", "\\}") .. "}")
  end
  local mid_tpl_f, er2 = template.compile_file("tpl/"..pub.."/midamble.tpl", kv)
  if mid_tpl_f then
    spit("gen/midamble.inc.tex", mid_tpl_f)
  else
    spit("gen/midamble.inc.tex", er2)
  end
  local pst_tpl_f, er3 = template.compile_file("tpl/"..pub.."/postamble.tpl", kv)
  if pst_tpl_f then
    spit("gen/postamble.inc.tex", pst_tpl_f)
  else
    spit("gen/postamble.inc.tex", er3)
  end
end
